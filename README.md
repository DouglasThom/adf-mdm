...
Use ADF to run a watermark-based REST API batch. Each run requests a bounded delta window from IBM MDM entity history or export APIs, lands raw responses in Azure storage, processes results in blocks below the confirmed API limit, and writes one curated output row per changed player ID to Snowflake.
...
- ADF will orchestrate REST API calls, pagination, block processing, Snowflake loading, and watermark storage.
